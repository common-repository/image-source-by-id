...
use JLTISRC\JLT_Image_Source_ID;
...
if ( ! class_exists( 'Upgrader' ) ) {
	...
	class Upgrader {
		...
		public function __construct() {
			$this->option_name = JLT_Image_Source_ID::plugin_version_key();
		}
		...
		protected function get_installed_version() {
			return get_option( $this->option_name, JLTISRC_VER );
		}
		...
		public function if_updates_available() {
			if ( version_compare( $this->get_installed_version(), JLTISRC_VER, '<' ) ) {
				return true;
			}
		...
		public function run_updates() {
			...
			$path              = trailingslashit( JLTISRC_DIR );
